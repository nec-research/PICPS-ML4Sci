...
import ocpmodels.common.utils as oc_utils
...
class AtomsToGraphs:
    ...
    def convert(
        ...
        Returns:
            data (torch_geometric.data.Data): A torch geometic data object with r__index_index, positions, atomic_numbers,
        ...
        if self.r_edges:
            #edge_index, cell_offsets, edge_distances, _ = oc_utils.radius_graph_pbc(
            edge_index, cell_offsets, edge_distances = oc_utils.radius_graph_pbc(
                data, self.radius, self.max_neigh)
            # edge_distances is actually not edge_distances but image
