...
class spinconv(BaseModel):
    ...
    def forward(self, data, if_rotation=False):
        ...
        outputs = self._forward_helper(
            data, edge_index, edge_distance, edge_distance_vec, if_rotation=if_rotation
        )
    ...
    @conditional_grad(torch.enable_grad())
    def _forward_helper(
        self, data, edge_index, edge_distance, edge_distance_vec, if_rotation=False
    ):
        ###############################################################
        # Initialize messages
        ###############################################################
        ...
        if self.regress_forces:
            ...
            if False: #if_rotation:   # <-- this return Nan maybe due to random rotation and summation in "random"
                dFx = (
                    torch.autograd.grad(
                        forces[..., 0],
                        data.pos,
                        grad_outputs=torch.ones_like(forces[..., 0]),
                        create_graph=True,
                    )[0]
                )
                dFy = (
                    torch.autograd.grad(
                        forces[..., 1],
                        data.pos,
                        grad_outputs=torch.ones_like(forces[..., 1]),
                        create_graph=True,
                    )[0]
                )
                dFz = (
                    torch.autograd.grad(
                        forces[..., 2],
                        data.pos,
                        grad_outputs=torch.ones_like(forces[..., 2]),
                        create_graph=True,
                    )[0]
                )
                rotation_x = dFy[..., 2] - dFz[..., 1]
                rotation_y = dFz[..., 0] - dFx[..., 2]
                rotation_z = dFx[..., 1] - dFy[..., 0]
                rotation = torch.sqrt(
                    rotation_x ** 2 + rotation_y ** 2 + rotation_z ** 2
                )
                return energy, forces, rotation
            else:
