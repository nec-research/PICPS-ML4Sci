...
```
from mdsim.modules import Evaluator
...
class Evaluator:
    ...
    def __init__(self, task=None, no_energy=False, forces_rotation=False):
        assert task in ["s2ef"]
        self.forces_rotation = forces_rotation
        if forces_rotation:
            self.task_metrics = {
                "s2ef": [
                    "forces_mae",
    ...
    def update(self, key, stat, metrics):
        ...
        if key.split('_')[-1] == "rmse":
            metrics[key]["metric"] = np.sqrt(metrics[key]["metric"])
...
def rmse(prediction, target):
    ...
    error = (target - prediction) ** 2
    return {
        "metric": torch.sqrt(torch.mean(error)).item(),
        "total": torch.sum(torch.sqrt(error)).item(),
        "numel": prediction.numel(),
    }
...
def forces_rotation(prediction, target):
    return {
        "metric": torch.mean(prediction["rotation"]).item(),
        "total": torch.sum(prediction["rotation"]).item(),
        "numel": prediction["rotation"].numel(),
    }
